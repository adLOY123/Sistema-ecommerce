import { Search, ShoppingCart, Heart, User, Menu, X }
    from "lucide-react"
        ...
        <header className="fixed top-0 left-0 w-full z-50 bg-orange-600 shadow-md">
            <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8">
                <div className="flex items-center justify-between h-20 gap-4">
                    ...
                    {isSearchOpen && (
                        <div className="absolute inset-0 z-50 bg-orange-600 flex items-center px-4 sm:hidden">
                          ...
                          <div className="flex-1 flex items-center bg-white rounded-full  overflow-hidden h-11 shadow-sm">
                            ...
                            <button className="px4 text-orange-600 hover:text-orange-800 transition-colors cursor-pointer">
                                <Search size={20}/>
                            </button>
                          </div>
                          <button onClick={()=> setIsSearchOpen(false)} className="ml-3 text-white cursor-ponter p-1">
                            <X size={28}/>
                    ...
                    {/* Centro: buscador */}
                    <div className="hidden sm:flex flex-1 max-w-2xl bg-white rounded-full overflow-hidden h-11 shadow-sm">
                        <input
                            type="text"
